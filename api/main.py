...
# Checkup Health risk
MODEL_RISK = joblib.load("random_forest_model.joblib")
...
async def recognize_and_parse_prescription(file: UploadFile = File(...)):
    """
    Recognizes and parses prescription details from an uploaded file.
    
    Parameters:
        file (UploadFile): The prescription file uploaded by the user.
    
    Returns:
        dict: A dictionary containing the recognized text and parsed prescription information.
    """
    ...
    try:
        ...
        # Perform OCR to recognize text from the uploaded prescription file
...
def parse_prescription(text: str) -> List[Tuple[str, str]]:
    """
    Parses prescription details from the recognized text using SpaCy.
    
    Parameters:
        text (str): The recognized text from the prescription.
    
    Returns:
        List[Tuple[str, str]]: A list of tuples containing extracted entities (drug names, quantities, times).
    """
...
def generate_schedule(parsed_info: List[Tuple[str, str]]) -> List[datetime]:
    """
    Generates a schedule for drug administration based on TIME values extracted from the prescription.
    
    Parameters:
        parsed_info (List[Tuple[str, str]]): The parsed prescription information containing drug names and times.
    
    Returns:
        List[datetime]: A list of datetime objects representing the schedule for drug administration.
    """
...
# Predict Heart condition (High Accurate)
